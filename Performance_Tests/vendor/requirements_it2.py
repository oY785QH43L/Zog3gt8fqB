...
def v1() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + "/account/create"
        body = {
...
def login():
    """Logs the vendor in."""
    try:
        url = vendor_test_data.it2_prefix + "/account/login"
...
def v2() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + f"/account/update/{str(v2_data["id"])}"
        body = {
            "vendorId": v2_data["id"],
...
def v3() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + f"/account/{str(v3_data["id"])}"
        start = time.time()
...
def v4() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + f"/account/{str(v4_data["id"])}"
        start = time.time()
...
def v5() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + "/address/create"
        body = {
            "vendorId": v5_data["id"],
...
def v6() -> float:
    ...
    try:
        ...
        url = (
            vendor_test_data.it2_prefix + f"/address/update/{str(v6_data["addressId"])}"
        )
        body = {
            "vendorId": v6_data["id"],
...
def v8() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + f"/address/{str(v8_data["id"])}"
        start = time.time()
        response = requests.get(url)
        end = time.time()
...
def v9() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + "/product/create"
        body = {
            ...
            "categories": json.dumps(v9_data["categories"]),
        }

        with open(v9_data["imageContent"], "rb") as image_content, open(
            v9_data["videoContent"], "rb"
        ) as video_content:
            files = {"imageContent": image_content, "videoContent": video_content}
            start = time.time()
...
def v10() -> float:
    ...
    try:
        ...
        with open(v10_data["imageContent"], "rb") as image_content, open(
            v10_data["videoContent"], "rb"
        ) as video_content:
            files = {"imageContent": image_content, "videoContent": video_content}
            start = time.time()
            response = requests.put(url, data=body, files=files)
            end = time.time()
...
def v12() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + f"/product/{str(v12_data["id"])}"
        start = time.time()
        response = requests.get(url)
        end = time.time()
...
def v13() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + f"/product/addcategory"
        body = {
...
def v14() -> float:
    ...
    try:
        ...
        url = vendor_test_data.it2_prefix + f"/product/removecategory"
        body = {
