...
export class AdminService {
    ...
    public async createNewCourierAddress(address: Address, courier: Courier): Promise<Address> {
        ...
        try {
            ...
            this.loggerService.logInfo(`Fetched Address with ID '${address.addressId}'.`, "AdminService-MSSQL");
    ...
    public async updateCourierAddress(address: Address, courier: Courier): Promise<Address> {
        ...
        try {
            ...
            this.loggerService.logInfo(`Fetched CustomerOrder with data ${JSON.stringify({ billingAddressId: address.addressId })}.`, "AdminService-MSSQL");
            ...
            this.loggerService.logInfo(`Deleted Address with data ${JSON.stringify({ addressId: address.addressId })}.`, "AdminService-MSSQL");
            await connection.close();
            return addressToReturn;
        }
        catch (err) {
    ...
    public async deleteCourierAddress(addressId: number, courierId: number): Promise<void> {
        ...
        try {
            ...
            this.loggerService.logInfo(`Fetched CustomerOrder with data ${JSON.stringify({ addressId: addressId })}.`, "AdminService-MSSQL");
            ...
            this.loggerService.logInfo(`Deleted Address with data ${JSON.stringify({ addressId: addressId })}.`, "AdminService-MSSQL");
            await connection.close();
        }
        catch (err) {
            await connection.close();
