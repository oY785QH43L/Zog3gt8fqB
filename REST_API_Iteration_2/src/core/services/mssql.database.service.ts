...
export class MssqlDatabaseService {
    /**
     * Initializes the MSSQL database service.
    ...
    public async getNewId(entityName: string, attributeName: string): Promise<Number> {
        ...
        try {
            ...
            if (id == null) {
                this.loggerService.logInfo(`Generated new value for attribute '${attributeName}' in entity '${entityName}' - ${toReturn}.`, "MssqlDatabaseService");
                return toReturn;
            }

            await connection.close();
            toReturn = Number(id) + 1;
            this.loggerService.logInfo(`Generated new value for attribute '${attributeName}' in entity '${entityName}'- ${toReturn}.`, "MssqlDatabaseService");
            return toReturn;
        }
        catch (err) {
