...
export class CustomersService {
    ...
    public async updateCustomerAddress(address: Address, customer: Customer): Promise<Address> {
        ...
        try {
            ...
            this.loggerService.logInfo(`Fetched CourierToAddress with data ${JSON.stringify({ addressId: address.addressId })}.`, "CustomersService-MSSQL");
    ...
    public async deleteCustomerAddress(addressId: number, customerId: number): Promise<void> {
        ...
        try {
            ...
            this.loggerService.logInfo(`Fetched CourierToAddress with data ${JSON.stringify({ addressId: addressId })}.`, "CustomersService-MSSQL");
    ...
    public async getCourierInformation(courierId: number): Promise<CourierInformation> {
        ...
        try {
            ...
            this.loggerService.logInfo(`Fetched Courier with ID '${courierId}'.`, "CustomersService-MSSQL");
            ...
            this.loggerService.logInfo(`Fetched Address for Courier with ID '${courierId}'.`, "CustomersService-MSSQL");
            let addressesConverted: Address[] = addresses[0].map(function(v) {
                return { addressId: v["AddressId"], street: v["Street"], city: v["City"], postalCode: v["PostalCode"], country: v["Country"] } as Address;
            });
            let result = {
    ...
    public async makeOrder(shoppingCartId: number, billingAddressId: number, courierCompanyId: number): Promise<CustomerOrder> {
        ...
        try {
            ...
            this.loggerService.logInfo(`Fetched Courier with ID '${courierCompanyId}'.`, "CustomersService-MSSQL");
